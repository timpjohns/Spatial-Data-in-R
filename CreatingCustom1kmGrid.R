# Created by Timothy Johnson. Last updated: 11/16/2015
...
"rgdal","raster","R.utils","spsurvey", "xlsx", "rJava", "foreign", "stringr"),dep=TRUE)

#load libraries
library(spatstat); library(maptools); library(lattice); library(sp); library(RColorBrewer); 
library(splancs); library(maps); library(rgdal); library(raster); library(R.utils); 
library(spsurvey); library(foreign); library(rJava); library(xlsx); library(plyr); library(stringr)

#Read 1km raster (change filename to what you named it)
kmgrid = readGDAL("cell5m_Ghana_1km2.tif")

#convert 1km raster to data frame
kmgridx= as.data.frame(kmgrid, row.names=NULL, optional=FALSE, xy=FALSE, na.rm=TRUE)

#sort by all three columns in this order: band1, y, x. This ensures that the data 
#will all be renamed in the same way starting in the lower left corner.  
sortedGrid = kmgridx[with(kmgridx, order(band1, y, x)), ]
sort

#view the column of interest, make sure it is reading properly (optional) 
sortedGrid$band1
...
#export to comma separated text file to load into ArcGIS as points. Convert points to raster, 
#snapping to the 5m grid and setting cell size to 1km. Also make sure values are
